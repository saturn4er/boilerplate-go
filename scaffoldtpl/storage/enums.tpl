...
{{- define "storage.enum.values" }}
  const (
    {{- range $value := .Values }}
        {{ template "storage.const.enum_value" (dict "enum" $ "value" $value)}} = "{{$value | snakeCase}}"
    {{- end}}
  )
{{- end }}
...
{{- define "storage.enum.to_db_converter"}}
  ...
  func {{template "storage.storage.func.db_enum_to_enum" .Name}}({{$argName}} {{$enumGoType.Ref}}) (string, error){
...
{{- define "storage.enum.from_db_converter"}}
    ...
    {{- $enumPkg := $enumGoType.PackageImport }}
    {{- $argName := print (.Name| lCamelCase) "Value" }}
    func {{template "storage.func.db_enum_to_enum" .Name}}({{$argName}} string) ({{$enumGoType.Ref}}, error){
